...
class BackupApp(QMainWindow):
    ...
    def show_progress_bar(self, total_size):
    ...
    def update_progress(self, file_size):
        ...
        if self.progress_bar.isVisible():
            ...
            progress = int((self.copied_size / self.current_backup_size) * 100)
            self.progress_bar.setValue(progress)
            copied_mb = self.copied_size / (1024 * 1024)
            total_mb = self.current_backup_size / (1024 * 1024)
            self.status_label.setText(f"Копирование... ({copied_mb:.1f} MB / {total_mb:.1f} MB)")
    ...
    def validate_backup_conditions(self):
        ...
        # Проверяем, что хотя бы одна исходная папка/файл существует
        valid_sources = False
        for folder in self.source_folders:
            if os.path.exists(folder):
                valid_sources = True
        ...
        if not valid_sources:
            self.log_message("Проверка условий: ни один исходный файл/папка не существует")
            return False
    ...
    def manual_backup(self):
        """Выполнение ручного резервного копирования"""
        if not (self.source_folders or self.source_files) or not self.destination_folder:
            QMessageBox.warning(self, "Ошибка", "Выберите исходные файлы/папки и папку назначения!")
            return
    ...
    def perform_backup(self):
        """Основная логика выполнения резервного копирования"""
        try:
            if not self.validate_backup_conditions(): 
                return
            
            if hasattr(os, 'statvfs'):  # Linux/Mac
            ...
            if total_backup_size > free_space:
                self.log_message("✗ Недостаточно свободного места для копирования")
                return
            ...
            if copied_count > 0:
                self.log_message(f"✓ Скопировано {copied_count} файлов ({total_size/1024:.2f} KB)")
