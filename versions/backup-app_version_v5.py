...
class BackupApp(QMainWindow):
    ...
    def setup_custom_statusbar(self):
        ...
        # Устанавливаем статус бар внизу главного окна
        layout = self.centralWidget().layout()
        layout.addWidget(status_widget)
    
    def show_progress_bar(self, total_size):
        """Показывает прогресс бар для большого копирования"""
        # Показываем прогресс бар только если общий размер больше 10MB
        if total_size > 10 * 1024 * 1024:  # 10MB в байтах
    ...
    def update_progress(self, file_size):
        ...
        if self.progress_bar.isVisible():
            ...
            self.status_label.setText(f"Копирование... ({self.copied_size//1024} KB / {self.current_backup_size//1024} KB)")
    ...
    def perform_backup(self):
        try:
            ...
            # Вычисляем общий размер копирования
